#include "../includ_test/s21_test.h"
...
START_TEST(floats_sci2) {
  ...
  // К сожалению, утверждения для inf некорректно работают в libcheck
...
Suite *suite_sscanf(void) {
  ...
  // [EOF]
  // Flags: ничего не делать (неприменимо к данному спецификатору)
  ...
  // // [%c]
  // // Flags:
  // // [Width] - N: 0, 1 - Ничего не делать; N > 1 - Неопределенное поведение.
  // //          * - пропускает символы
  // // [Precision] - Любой флажок точности немедленно останавливает работу
  // sscanf (error)
  // // [Length] - Ничего не делает
  ...
  tcase_add_test(tc, chars_flags1);  // Флаги точности приводят к сбою
  tcase_add_test(tc, chars_flags2);  // Флаги ширины ничего не делают (if <= 1)
  tcase_add_test(
      tc,
      chars_flags3);  // Демонстрирует, как работает (*). * - игнорируйте символ
  tcase_add_test(tc, chars_flags4);  // Флаги длины ничего не делают
  tcase_add_test(tc, chars_flags5);  // * флаги - игнорирует символ
  // Тестовые примеры специально для (%*c), так как это очень важная концепция
  tcase_add_test(tc, chars_aster1);
  tcase_add_test(tc, chars_aster2);

  // // [%d]/[%i]
  // // [Width]: * - пропустить токен, (N) - прочитать N символов как число
  // // [Precision]: Не применимо к int, рассматриваемому как UB
  ...
  // // [%f] [%g] [%G]
  // // Width: (N) - parses N chars, (*) - пропустить строку
  // // Precision: учитывайте это при разборе AFTER (.)
  // // Length - l - long
