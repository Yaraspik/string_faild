...
void write_int_to_memory(char **str, int *fail_flag, int *res, token *tok) {
  ...
  *fail_flag = 1;
  if (s21_strspn(*str, "0123456789+-")) {
    // Libc sscanf не разрешает "++n" / "--f" и немедленно прерывает
    int sign = s21_strspn(*str, "+-");
    // Libc sscanf засчитывает +- в ширину. Значение Signed int с шириной <= 1
    // невозможно
...
// если значение BUFF равно S21_NULL, мы просто пропускаем символы, если
// точность <= 0, это ничего не дает
...
void write_float_to_memory(char **str, int *res, token *tok) {
  ...
  if (tok->width != WIDTH_NUMBER) {
    if (tok->spec == 'f')
      ...
      write_chars_to_buff(str, ".0123456789+-NaAifIFn", S21_NULL, 0, 0);
  }
}
